...
export default {
  ...
  async mounted() {
    const sampleWorker = new SampleWorker();
    const sampleWorkerObj = Comlink.proxy(sampleWorker);
    const interval = setInterval(async () => {
      console.log('Counting...');
      await sampleWorkerObj.incrementBy(
        this.inputs.increment,
        Comlink.proxyValue((val) => {
          if (val === this.inputs.maxValue) {
            console.log('Counter Finished: ', val);
